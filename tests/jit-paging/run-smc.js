...
var test_executable = new Uint8Array(fs.readFileSync(__dirname + "/test-jit-smc"));
...
emulator.bus.register("emulator-started", function()
{
    ...
    emulator.create_file("test-jit-smc", test_executable);
});
...
emulator.add_listener("serial0-output-byte", async function(byte)
{
    ...
    {
        ran_command = true;
        emulator.serial0_send("chmod +x /mnt/test-jit-smc\n");
        emulator.serial0_send("/mnt/test-jit-smc 2>&1 | tee /mnt/result\n");
    ...
    if(new_line && new_line.includes("test finished"))
    {
        ...
        if(result !== "test_smc_multipage passed\n")
        {

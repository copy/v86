...
- If you have Docker (or Docker Desktop) installed, you can run the whole system inside a container.
- See `tools/docker/exec` to find Dockerfile required for this.
- You can run `docker build -f tools/docker/exec/Dockerfile -t v86:alpine-3.14 .` from root directory to generate docker image.
- Then you can simply run `docker run -it v86:alpine-3.14` to start the server.

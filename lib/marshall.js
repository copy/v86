    ...
    for(var i=0; i < typelist.length; i++) {
        ...
        switch(typelist[i]) {
            ...
            case "s":
                ...
                var string_bytes = texten.encode(item);
                size += string_bytes.byteLength;
                length += string_bytes.byteLength;
                struct.set(string_bytes, offset);
                offset += string_bytes.byteLength;
    ...
    for(var i=0; i < typelist.length; i++) {
        switch(typelist[i]) {
            ...
            case "s":
                ...
                var string_bytes = struct.slice(offset, offset + len);
                offset += len;
                output.push(textde.decode(string_bytes));
                break;
            case "Q":
                state.offset = offset;

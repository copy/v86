...
// Compatibility shims for the previous (non-module) global helpers that
// 9p-filer.js relied on (formerly provided by jor1k.js / lib.js globals).
const hex8 = (n) => h(n, 2);
const message = {
    /** @param {...*} args */
    Debug: function(...args) {
        dbg_log(args.join(" "), LOG_9P);
    },
    Abort: function() {
        dbg_assert(false, "message.Abort()");
    },
};
const v86util = {
    range: (size) => Array.from(Array(size).keys()),
};
 ...
 * @constructor
 *
 * @param {!Object} filesystem A Filer-backed filesystem ({ fs, sh, Path, Buffer })
 * @param {CPU} cpu
 * @param {BusConnector} bus
 */
export function Virtio9pFiler(filesystem, cpu, bus) {
...
Virtio9pFiler.prototype.shouldAbortRequest = function(tag) {
...
Virtio9pFiler.prototype.get_state = function() {
    var state = [];
...
Virtio9pFiler.prototype.set_state = function(state) {
...
Virtio9pFiler.prototype.Createfid = function(path, type, uid) {
    return { path, type, uid };
};


Virtio9pFiler.prototype.Reset = function() {
    this.fids = {};
};

// Alias to match the interface expected by CPU.reset_all() (lib/9p.js uses
// the lowercase name).
Virtio9pFiler.prototype.reset = Virtio9pFiler.prototype.Reset;

// Before we begin any async file i/o, mark the tag as being pending
Virtio9pFiler.prototype.addTag = function(tag) {
    this.pendingTags[tag] = {};
};

// Flush an inflight async request
Virtio9pFiler.prototype.flushTag = function(tag) {
    delete this.pendingTags[tag];
};

Virtio9pFiler.prototype.BuildReply = function(id, tag, payloadsize) {
...
Virtio9pFiler.prototype.SendError = function(tag, err) {
...
Virtio9pFiler.prototype.SendReply = function(bufchain) {
...
Virtio9pFiler.prototype.ReceiveRequest = async function(bufchain) {
    var self = this;
    var Path = this.Path;

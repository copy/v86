...
Virtio9p.prototype.ReceiveRequest = function (bufchain) {
    ...
    switch(id)
    {
        ...
        case 52: // lock
            ...
            var fid = req[0];
            var flags = req[2];
            var lock_request = this.fs.DescribeLock(req[1], req[3], req[4], req[5], req[6]);
            message.Debug("[lock] fid=" + fid +
                ", type=" + lock_request.type + ", start=" + lock_request.start +
        ...
        case 54: // getlock
            ...
            var fid = req[0];
            var lock_request = this.fs.DescribeLock(req[1], req[2], req[3], req[4], req[5]);
            ...
            size = marshall.Marshall(["b", "d", "d", "w", "s"],
                [ret.type, ret.start, ret.length, ret.proc_id, ret.client_id],
                this.replybuffer, 7);

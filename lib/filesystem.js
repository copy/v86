...
/**
 * @private
 * @param {Inode} directory
 * @return {boolean}
 */
FS.prototype.directory_is_linkable = function(directory)
{
    // Note: Non-root forwarder inode could still have a non-forwarder parent, so don't use
    // parent inode to check.
    return !this.is_forwarder(directory) || directory.foreign_id === 0;
};
...
FS.prototype.link_under_dir = function(parentid, idx, name)
{
    ...
    const real_inode = this.GetInode(idx);
    ...
    dbg_assert(real_inode.nlinks >= 0,
        "Filesystem: Found negative nlinks value of " + real_inode.nlinks);
    ...
    // Root inode of other filesystems are already linked to themselves.
    if(real_inode.fid !== 0)
    {
        // Never store link information on forwarder.
        real_inode.nlinks++;
    }
    ...
    {
        // Safe assumption:
        // We will never link a directory forwarder apart from the mountpoint.
        // Directories cannot be hardlinked to two places,
        // so the parent of any non-root directory forwarder must also be a forwarder parent,
        // but forwarder parents are already asserted against.
        dbg_assert(this.directory_is_linkable(inode),
            "Filesystem: Cannot link non-root directory forwarder");
        dbg_assert(!inode.direntries.has(".."),
            "Filesystem: Cannot link a directory twice");

        if(!real_inode.direntries.has(".")) real_inode.nlinks++;
...
FS.prototype.unlink_from_dir = function(parentid, name)
{
    ...
    dbg_assert(exists, "Filesystem: Can't unlink non-existent file: " + name);

    // Root inode of other filesystems are still linked to themselves.
    if(real_inode.fid !== 0)
    {
        // Never store link information on forwarder.
        real_inode.nlinks--;
...
FS.prototype.divert = function(parentid, filename)
{
    ...
    dbg_assert(this.IsDirectory(old_idx) || this.GetInode(old_idx).nlinks <= 1,
        "Filesystem: can't divert hardlinked file '" + filename + "' with nlinks=" +
        this.GetInode(old_idx).nlinks);
...
FS.prototype.Rename = function(olddirid, oldname, newdirid, newname) {
    ...
    else if(this.GetInode(idx).fid === 0)
    {
    ...
    else if(!this.IsDirectory(idx) && this.GetInode(idx).nlinks > 1)
    {
        ...
        if(!this.is_forwarder(inode))
        {
            dbg_log("XXX: Attempted to move hardlinked file (" + oldname + ") " +
                "to a child filesystem - skipped", LOG_9P);
            return -EPERM;
        }
        if(this.is_forwarder(newdir) && newdir.mount_id !== inode.mount_id)
        {
            dbg_log("XXX: Attempted to move hardlinked file (" + oldname + ") " +
                "to a sibling filesystem - skipped", LOG_9P);
            return -EPERM;
        }

        const link_ret = this.Link(newdirid, idx, newname);
        if(link_ret < 0) return link_ret;

        const unlink_ret = this.Unlink(olddirid, oldname);
        if(unlink_ret < 0) return unlink_ret;
    }
    else
    {
...
FS.prototype.GetParent = function(idx)
{
    ...
    if(this.directory_is_linkable(inode))
    {

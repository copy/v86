...
FS.prototype.IsLoading = function()
{
    if(this.filesinloadingqueue > 0) return true;
    for(const mount of this.mounts)
    {
        if(mount.fs.IsLoading()) return true;
    }
    return false;
};
...
FS.prototype.LoadFile = function(idx) {
    ...
    if(this.is_forwarder(inode))
    {
        // Events handled by child filesystem.
        this.follow_fs(inode).LoadFile(inode.foreign_id);
        return;
    }
 ...
 */
FS.prototype.link_under_dir = function(idx, parentid)
{
    const inode = this.inodes[idx];
    const parent_inode = this.inodes[parentid];
    dbg_assert(!this.is_forwarder(parent_inode), "Filesystem: Shouldn't link non-root fowarders");
    inode.parentid = parentid;
    inode.nextid = parent_inode.firstid;
    parent_inode.firstid = idx;
 ...
 * @param {number} idx
 */
FS.prototype.unlink_from_dir = function(idx)
{
    const inode = this.inodes[idx];
    const parent_inode = this.inodes[inode.parentid];
    dbg_assert(!this.is_forwarder(parent_inode), "Filesystem: Can't unlink non-root fowarders");
    if(parent_inode.firstid === idx)
    {
        parent_inode.firstid = inode.nextid;
    }
    else
    {
        const id = this.FindPreviousID(idx);
        dbg_assert(id !== -1,
            "Filesystem: Could not find previous sibling of (" + this.GetInode(idx).name +
            ") while unlinking from it's parent");
...
FS.prototype.PushInode = function(inode) {
    if (inode.parentid != -1) {
        ...
        this.link_under_dir(this.inodes.length - 1, inode.parentid);
        return;
    } else {
 ...
 * @private
 * @param {number} old_idx Target to divert.
 * @return {number} New idx of diversion.
 */
FS.prototype.divert = function(old_idx)
{
    dbg_assert(old_idx !== 0, "Filesystem: can't divert root inode");
    dbg_assert(old_idx > 0, "Filesystem: invalid inode idx: " + old_idx);
    ...
    {
        this.unlink_from_dir(old_idx);
        this.link_under_dir(idx, old_inode.parentid);
    }
    ...
    if(this.IsDirectory(old_idx) && !this.is_forwarder(old_inode))
    {
        ...
        while(child_id !== -1)
        {
            this.inodes[child_id].parentid = idx;
...
FS.prototype.Rename = function(olddirid, oldname, newdirid, newname) {
    ...
    if(!this.is_forwarder(olddir) && !this.is_forwarder(newdir))
    {
        ...
        this.unlink_from_dir(idx);
        this.link_under_dir(idx, newdirid);
    ...
    else
    {
        ...
        const diverted_old_idx = this.divert(idx);
        ...
        if(this.is_forwarder(newdir))
        {
            ...
            const foreign_id = foreign_fs.CreateFile(newname, newdir.foreign_id);
            const new_real_inode = foreign_fs.GetInode(foreign_id);

            // Copy over inode info. Reassign info we want to keep.
            Object.assign(new_real_inode, old_real_inode,
            {
                fid: new_real_inode.fid,
                parentid: new_real_inode.parentid,
                nextid: new_real_inode.nextid,
                firstid: -1,
                name: newname,
            });
        ...
        else
        {
            ...
            Object.assign(inode, old_real_inode);
...
FS.prototype.Link = function(targetid, name, parentid)
{
    ...
    if(this.is_forwarder(this.inodes[parentid]))
    {
        inode.parentid = 0;
        const newid = this.inodes.length;
        this.PushInode(inode);
        this.Rename(0, name, parentid, name);
    }
    else
    {
        inode.parentid = parentid;
...
FS.prototype.Unlink = function(idx) {
    ...
    if (idx == 0) return -EPERM; // root node cannot be deleted
    var inode = this.inodes[idx];
    ...
    {
        const foreign_id = inode.foreign_id;
        const ret = this.follow_fs(inode).Unlink(foreign_id);
        if(ret === 0) this.delete_forwarder(inode);
        return ret;
    }

    // check if directory is not empty
    if ((inode.mode&S_IFMT) == S_IFDIR) {
       if (inode.firstid != -1) return -ENOTEMPTY;
    }
...
FS.prototype.DeleteData = function(idx)
{
    const inode = this.inodes[idx];
    if(this.is_forwarder(inode))
    {
...
FS.prototype.GetRecursiveList = function(dirid, list) {
    if(this.is_forwarder(this.inodes[dirid]))
    {
        ...
        const foreign_list = [];
        foreign_fs.GetRecursiveList(foreign_dirid, foreign_list);
        for(const foreign_id of foreign_list)
        {
            list.push(this.get_forwarder(this.inodes[dirid].mount_id, foreign_id));
        }
        return;
    }
...
FS.prototype.RecursiveDelete = function(path) {
    ...
    {
        const ret = this.Unlink(toDelete[i]);
        dbg_assert(ret === 0, "Filesystem RecursiveDelete failed at inode=" + toDelete[i] +
            " with error code: " + (-ret));
    }

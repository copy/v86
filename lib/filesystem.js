...
FS.prototype.RecursiveDelete = function(path) {
    ...
    this.GetRecursiveList(ids.id, to_delete);

    for(var i=to_delete.length-1; i>=0; i--)
    {
        const ret = this.Unlink(to_delete[i].parentid, to_delete[i].name);
        dbg_assert(ret === 0, "Filesystem RecursiveDelete failed at parent=" + to_delete[i].parentid +
            ", name='" + to_delete[i].name + "' with error code: " + (-ret));
    }
};
